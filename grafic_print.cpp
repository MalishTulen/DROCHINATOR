    ...
    for ( int i = 0; i < DATA_CAPACITY; i++ )
    {
        int checker_if_leaf = check_if_leaf ( &ptr_data->ptr_tree[ i ] );
            if ( checker_if_leaf == LEAF )
                fprintf ( ptr_dot_file, "node%p[ shape = record, label = \" {<f0> NODE%d(%p) | <f1> %s | <f2> prev = (%p) | { <f3> ДА ( %p ) | <f4> ПИЗДА ( %p ) } } \"];\n", &ptr_data->ptr_tree[ i ], i, &ptr_data->ptr_tree[ i ], ptr_data->ptr_tree [ i ].que_or_answ,ptr_data->ptr_tree [ i ].prev, ptr_data->ptr_tree [ i ].right_branch, ptr_data->ptr_tree [ i ].left_branch );
...
errors_t make_picture ( const char* dot_file, const char* output_file, int* amount_of_pictures )
{
    ...
    sprintf ( cmd_command, "dot %s -T png -o dot_files/%s%d.png", dot_file, output_file, *amount_of_pictures );
...
//fprintf ( stderr, "POSIT = '%p'\n", ptr_index->left_branch );
    if ( ptr_index->left_branch != NULL)
    {
        fprintf ( ptr_dot_file, "node%p:<f3>->node%p\n", ptr_index, ptr_index->left_branch );
        branch_connector ( ptr_index->left_branch, ptr_data, ptr_dot_file );
        //checker_if_leaf += 2;
    }
    if ( ptr_index->right_branch != NULL)
    {
        fprintf ( ptr_dot_file, "node%p:<f4>->node%p\n", ptr_index, ptr_index->right_branch );
        branch_connector ( ptr_index->right_branch, ptr_data, ptr_dot_file );
        //checker_if_leaf++;
    }
